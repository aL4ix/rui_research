...
use crate::widgets::themes::style::{ButtonStyle, ImageStyle, Style, TextBoxStyle, ThemeButtonStyle, ThemeImageStyle, ThemeTextBoxStyle};
use crate::widgets::themes::theme::{PrimitiveOrOneRef, Theme};

pub struct SimpleTheme;

impl Theme for SimpleTheme {
    fn for_button(&self, _size: Vector2D<f32>, text: &str, style: ThemeButtonStyle)
    ...
    fn for_text_box(&self, _size: Vector2D<f32>, text: &str, style: ThemeTextBoxStyle)
                    -> (Vector2D<f32>, Vec<PrimitiveOrOneRef<Text>>, Rc<Text>) {
        debug!("{:?}", style);
    ...
    fn for_image(&self, _size: Vector2D<f32>, path: Box<Path>, _style: ThemeImageStyle)
    ...
    fn style(&self) -> Vec<Box<dyn Style>> {
        vec![
            Box::new(ButtonStyle {
                ...
                font_size: 32.0,
                extra: vec![
                    ("color_background_gradient".to_string(), Color::new_opaque(0, 255, 0).into()),
                ],
                ..Default::default()
            }),

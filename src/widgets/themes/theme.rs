...
pub trait Theme {
    ...
    fn for_text_box(&self, size: Vector2D<f32>, text: &str, style: ThemeTextBoxStyle) -> (Vector2D<f32>, Vec<PrimitiveOrOneRef<Text>>, Rc<Text>);
    fn for_image(&self, size: Vector2D<f32>, path: Box<Path>, style: ThemeImageStyle) -> Result<SizePrimsAndRef<Bitmap>, Box<dyn Error>>;
